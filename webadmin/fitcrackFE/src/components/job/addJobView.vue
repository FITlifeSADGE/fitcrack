...
<template>
  <div class="containerAddJob height100 mx-auto">
    <v-scale-transition>
      <v-sheet v-show="keyspaceKnown && keyspace || estimatedTime" class="infobar" color="info" dark elevation="8">
        <div class="d-flex">
          <div v-show="keyspace" class="text-end">
            <div class="overline">
          ...
          <div v-show="estimatedTime" class="ml-4 text-end">
            <div class="overline">
              <v-icon x-small class="mr-1">
                mdi-timelapse
              </v-icon>
    ...
    <div v-show="loading" class="loadingProgressCont">
      <v-progress-circular indeterminate color="primary" size="65" class="loadingProgress" :width="4" />
    </div>

    <v-row>
      <v-col>
        <v-alert v-if="!helpAlreadyDismissed" text type="success" icon="mdi-help-box" dismissible>
          If you need help with creating a job, you can <a target="_blank"
            :href="$docsLink + '/#/jobs/creating/overview'">read the user manual here</a>.
          <template #close="{ toggle }">
            <v-btn text small @click="dismissHelp(toggle)">
              Don't show anymore
              <v-icon right>mdi-close</v-icon>
            </v-btn>
          </template>
        </v-alert>
        <v-alert v-if="helpDismissedMessage" text type="info" dismissible>
    ...
    <v-row>
      <v-col>
        <v-text-field v-model="name" outlined autofocus required label="Name" hint="Give this job a descriptive name"
          persistent-hint />
      </v-col>
      <v-col>
        <v-autocomplete :items="templates" item-text="name" item-value="id" :value="selectedTemplate" auto-select-first
          outlined label="Template" hint="Prefill the form with a saved template (use empty to quickly reset)"
          persistent-hint @input="fetchAndApplyTemplate" />
      </v-col>
    </v-row>
      ...
      <v-col>
        <v-stepper id="job-stepper" v-model="step" vertical non-linear class="mb-4">
          ...
          <v-stepper-content step="1">
            <v-container>
              <v-row class="mb-4">
                <v-btn-toggle v-model="inputMethod" mandatory color="primary" class="mr-2">
              ...
              <v-row>
                <v-col v-if="inputMethod === 'extractFromFile'" cols="12">
                ...
                <v-col v-if="inputMethod === 'hashFile'" cols="12">
                  <v-alert type="info" text>
                    Input to hashcat. Select a binary hash (WPA/WPA2), or plain text hashlist.
                  </v-alert>
                  <file-uploader ref="hashFileUploader" :multiple="false" no-upload label="Select a file to read"
                    @filesChanged="hashFileSelected" />
                </v-col>
                <v-col cols="12">
                  <v-alert :value="gotBinaryHash" text type="info" color="primary" class="mt-0 mb-0">
              ...
              <v-row>
                <v-btn v-if="dev" text color="success" @click="getRandomHash">
                  <v-icon left>
                    mdi-auto-fix
                  </v-icon>
                  Random SHA1
                </v-btn>
                <v-checkbox v-show="invalidHashes.length > 0" v-model="ignoreHashes" label="Ignore invalid hashes"
                  hide-details :height="15" color="error" class="ml-2 mt-1" />
                <v-spacer />
                <v-btn v-show="hashList !== ''" color="error" class="mr-2" text @click="clearInput">
                  Reset
                </v-btn>
          ...
          <v-stepper-content step="2">
            <v-container>
              ...
              <v-item-group v-model="attackSettingsTab" class="d-flex flex-wrap justify-space-between">
                <v-item v-for="(type, i) in attacks" :key="i" :value="type.handler" #default="{ active, toggle }">
                  <v-btn :id="`attack-mode-${type.handler}`" class="mode-btn" text small :color="active ? 'primary' : ''"
                    @click="toggle">
                    <div class="my-3">
                      <v-icon class="mb-2">
          ...
          <v-stepper-content step="3">
            <v-container>
              ...
              <div class="scroller">
                <host-selector v-model="hosts" select-all auto-refresh />
              </div>
              <v-subheader><b>({{ hosts.length }} selected)</b></v-subheader>
              <v-row>
                <v-spacer />
                <v-btn class="mr-6 mt-4" color="primary" @click="step = 4">
                  Next
                </v-btn>
              </v-row>
            </v-container>
          </v-stepper-content>
          <v-stepper-step id="job-step-4" editable step="4">
            Additional settings
          </v-stepper-step>
          <v-stepper-content step="4">
            <v-container>
              <v-row>
                <v-textarea id="comment-input" v-model="comment" label="Comment" outlined auto-grow :rows="1" />
              </v-row>
              <v-row>
                <v-col>
                  <div class="title mb-2">Planned start</div>
                  <dt-picker v-model="startDate" outlined hide-details :disabled="startNow" single-line label="" />
                  <v-checkbox v-model="startNow" label="Immediately" />
                </v-col>
                <v-col>
                  <div class="title mb-2">
                    Desired time per workunit
                  </div>
                  <v-text-field v-model="timeForJob" outlined hide-details single-line label="Time for job" required
                    type="number" suffix="seconds" :min="60" />
                </v-col>
                <v-col>
                  <div class="title mb-2">Planned end</div>
                  <dt-picker v-model="endDate" outlined hide-details :disabled="endNever" single-line />
                  <v-checkbox v-model="endNever" label="No time limit" />
                </v-col>
              </v-row>
            </v-container>
          </v-stepper-content>
        ...
        <v-row v-if="timeForJob < 180" justify="center">
          <v-alert outlined type="warning">
            Setting extremely low time per workunit leads to high overhead. The cracking may take much longer than
