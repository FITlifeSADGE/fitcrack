...
<template>
  <div class="height100">
    <v-navigation-drawer id="navigation-drawer" v-model="drawer" persistent class="navigationDrawer" enable-resize-watcher
      fixed app width="290">
      <router-link :to="{ name: 'home' }">
        <div class="logo">
          <img v-if="$vuetify.theme.dark" :src="require('@/assets/logo-dark.svg')" alt="logo">
          <img v-else :src="require('@/assets/logo-light.svg')" alt="logo">
        </div>
      </router-link>

      <div class="dash-link">
        <v-btn id="dash-link" to="/" text>
          <v-icon left>
            mdi-view-dashboard
          </v-icon>
          Dashboard
      ...
      <v-tabs v-model="navtab" icons-and-text grow class="notrans">
      ...
      <v-tabs-items v-model="navtab" class="notrans">
        <v-tab-item eager>
          <v-list id="nav-jobs-list" expand nav dense>
            <v-list-item v-if="$userCan('ADD_NEW_JOB')" :to="{ name: 'addJob' }">
              <v-list-item-action>
            ...
            <v-list-item :to="{ name: 'jobs' }" exact>
              <v-list-item-action>
            ...
            <v-list-item to="/bins/trash" exact>
              <v-list-item-action>
        ...
        <v-tab-item>
          <v-list id="nav-library-list" expand nav dense>
            ...
            <v-list-item :to="{ name: 'rainbowTables' }">
              <v-list-item-action>
                <v-icon>{{ routeIcon('rainbowTables') }}</v-icon>
              </v-list-item-action>
              <v-list-item-content>
                <v-list-item-title>Rainbow Tables</v-list-item-title>
              </v-list-item-content>
            </v-list-item>
          </v-list>
        </v-tab-item>
        <v-tab-item>
          <v-list id="nav-system-list" expand nav dense>
            ...
            <v-list-item v-if="$userCan('MANAGE_USERS')" :to="{ name: 'manageUsers' }">
              <v-list-item-action>
            ...
            <v-list-item v-if="$userCan('MANAGE_USERS')" :to="{ name: 'server' }">
              <v-list-item-action>
    ...
    <v-app-bar app height="64px" :class="['mainToolbar', { 'very-important-must-not-miss': daemonWarning }]"
      :color="daemonWarning ? 'error' : ''" :dark="daemonWarning">
      <v-app-bar-nav-icon id="nav-toggle" @click.stop="drawer = !drawer" />
      <v-toolbar-title>{{ $store.state.project }}</v-toolbar-title>
      <v-spacer />
      <v-row v-if="daemonWarning" class="align-center">
        <v-col class="text-center">
          <v-icon large left class="important-must-not-miss">
            mdi-alert-octagon
          </v-icon>
          <span class="mr-4">
            Some Daemons are not running!
          </span>
          <v-btn light @click="daemonDetails = true">
            Details
          </v-btn>
          <v-btn icon @click="daemonWarning = false">
            <v-icon>
              mdi-close-circle
            </v-icon>
          </v-btn>
      ...
      <v-spacer />
      <v-badge :value="notificationsCount > 0" :content="notificationsCount" color="red" overlap left top offset-y="18">
        <v-btn id="notifications-button" large rounded icon @click.stop="toggleNotifications">
          <v-icon>
            {{ notificationsCount > 0 ? 'mdi-bell' : 'mdi-bell-outline' }}
          </v-icon>
        </v-btn>
      </v-badge>
      <v-menu v-model="userFlyout" :close-on-content-click="false" transition="slide-y-transition">
        <template #activator="{ on }">
          <v-btn id="user-button" large rounded text class="pr-2 pl-3" v-on="on">
            <div class="text-none text-end subtitle-2 mr-2">
              {{ user.username }}
            </div>
            <v-avatar :color="daemonWarning ? 'secondary' : 'primary'" size="32">
        ...
        <v-card id="user-flyout" min-width="250">
          <v-card-text class="text--primary d-flex align-center">
            ...
            <v-spacer />
            <v-btn text class="ml-4" @click="roles = !roles">
              {{ user.role.name }}
              <v-icon right>
          ...
          <v-expand-transition>
            <div v-show="roles">
              <v-card-text class="py-0">
                ...
                <v-list dense>
                  <v-list-item v-for="role in roleList" :key="role">
          ...
          <v-card-actions>
            <v-btn text :to="{ name: 'myAccount' }" class="mr-2" @click="userFlyout = false">
              <v-icon left>
                {{ routeIcon('myAccount') }}
              </v-icon>
            ...
            <v-spacer />
            <v-btn text class="ml-2" @click.stop="$store.dispatch('signOut')">
              <span>Sign out</span>
              <v-icon right>
                mdi-logout-variant
              </v-icon>
      ...
      <v-btn large rounded icon color="accent" :href="$docsLink" target="_blank">
    ...
    <!-- NOTIFICATION DRAWER -->
    <v-navigation-drawer id="notifications-drawer" v-model="rightDrawer" class="pa-0" temporary right fixed app
      width="450">
      <notifications-wrapper ref="notifWrapper" :count="notificationsCount" @close="rightDrawer = false" />
    </v-navigation-drawer>
    <!-- D A E M O N (。>︿<) A L E R T -->
    <v-dialog v-model="daemonDetails" overlay-color="red darken-4" :overlay-opacity=".8" max-width="550">
      <v-card>
        <v-card-title>
          Daemons not running
        </v-card-title>
        <v-card-text class="pb-0">
          ...
          for the inner workings of Fitcrack. It seems that, unfortunately, some have crashed. Here's a list of inactive
          daemons:
        </v-card-text>
        <v-card-text class="py-0">
          <v-list dense>
            <v-list-item v-for="daemon in fallenDaemons" :key="daemon">
              <v-list-item-title>
                {{ daemon }}
              </v-list-item-title>
            </v-list-item>
        ...
        <v-card-text>
          <v-sheet dark color="error" class="pa-4 mb-2">
    ...
    <!-- TRANSFER SHORTCUT -->
    <v-fab-transition>
      <v-btn v-if="transferCount > 0 && $route.name !== 'transfer'" fab fixed bottom left class="z-top" color="primary"
        :to="{ name: 'transfer' }">
        <v-badge :content="transferCount" color="secondary">
          <v-icon>
            mdi-dolly
          </v-icon>
        </v-badge>
